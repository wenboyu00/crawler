...
class redissina(RedisCrawlSpider):
    name = "redissina"
    redis_key = 'redissina:start_urls'

    rules = (
        # follow all links
        Rule(LinkExtractor(), callback='parse', follow=True),
    )
    template = SQL.Sql.reach_Template("新浪新闻")

    def __init__(self, *args, **kwargs):
        # Dynamically define the allowed domains list.
        # domain = kwargs.pop('domain', '')
        # self.allowed_domains = filter(None, domain.split(','))
        super(redissina, self).__init__(*args, **kwargs)

    def parse(self, response):
        # http://api.roll.news.sina.com.cn/zt_list?channel=news&cat_1=gnxw&cat_2==gdxw1||=gatxw||=zs-pl||=mtjj&level==1||=2&show_ext=1&show_all=1&show_num=22&tag=1&format=json&page=1&callback=newsloadercallback&_=1505353908881
    ...
    def parse_new(self, response):
        ...
        title = response.css(self.template['Website_title']).extract_first()  # 标题
        if title == None:
            ...
            texts = []
            text = ''
            for p in response.css(self.template['Website_content']).extract()[:-1]:
                texts.append(p.strip())
                text = ''.join(texts)

            # 新闻源
            try:
                source = response.css(self.template['Website_source']).extract_first()
            except:
                print('Source ERROR,Source parser changed')
                source = response.css('.time-source span:text').extract_first()

            # 编辑人
            editor = response.css(self.template['Website_author']).extract_first().lstrip('责任编辑： ').strip()
            # 时间
            time = response.css(self.template['Website_pubtime']).extract_first().strip()
